...
public class CodingTree {
	 ...
	 */
	public CodingTree(List<Character> inputchars) {
	...
	private void construct(List<Character> inputchars) {
		...
		FrequencyCount frequency = new FrequencyCount(inputchars);
		count = frequency.getFrequencyMap();
		
		//iterate through map to make nodes and
		//put nodes into priority queue
		PriorityQueue<Node> nodequeue = makePriorityQueue(frequency.getFrequencyMap()); 
...
//		}
	...
	private PriorityQueue<Node> makePriorityQueue(
		...
		PriorityQueue<Node> out_queue = new PriorityQueue<Node>();
		Set<Character> charSet = frequencyMap.keySet();
		for(Character ichar : charSet) {
			out_queue.offer(new Node(frequencyMap.get(ichar), ichar));
		}
	...
	private void constructCodeBuilder(Node input) {
		if(input.left == null && input.right == null) {
			codeBuilder.append("\n"+ input.getCharacter() +" " + temp.toString());
			codes.add(new Code(input.getCharacter(), temp.toString()));
		}
